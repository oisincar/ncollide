...
fn first_interference_with_ray() {
    ...
    let ray = Ray::new(Point3::new(0.0, 1.8, 0.0), Vector3::new(1.0, 1.0, 0.0).normalize());

    // Ray misses first ball and hits second
    assert!(ball.toi_with_ray(&iso1, &ray, std::f32::MAX, true).is_none());
    assert!(ball.toi_with_ray(&iso2, &ray, std::f32::MAX, true).is_some());
    ...
    assert!((toi - toi_ball).abs() < 0.0001, "Ray collision with unexpected object");
}
